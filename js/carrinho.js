...
function assetPath(path) {
  return (window.location.pathname.includes('/catalogo/') ? '../' : '') + path;
}
...
function renderCartItems() {
  ...
  if (!container) return;
  if (cart.length === 0) {
    container.innerHTML = '<div class="cart-empty"><i class="fas fa-shopping-cart"></i><p>Seu carrinho está vazio</p><a href="produtos.html" class="btn btn-primary" id="continueShopping">Ver Produtos</a></div>';
  ...
  if (shippingLine) {
    if (ship > 0) {
      ...
      const csv = document.getElementById('cartShippingValue');
      if (csv) csv.textContent = 'R$ ' + ship.toFixed(2);
    } else if (shipping && ship === 0) {
      shippingLine.style.display = 'flex';
      const csv = document.getElementById('cartShippingValue');
      if (csv) csv.textContent = 'Grátis';
    } else {
      shippingLine.style.display = 'none';
    }
  }
  const ct = document.getElementById('cartTotal');
  if (ct) ct.textContent = 'R$ ' + getCartTotal().toFixed(2);
}
